...
export const predict = async (req, res, next) => {
  ...
  try {
    ...
    if (userStat && userData.length > 0) {
      ...
      const { gender, birthDay, height, weight, ...otherDetails } =
        userStat._doc;
      ...
      const averSpO2 = Math.round(
        spO2Values.reduce((sum, curr) => sum + curr, 0) / spO2Values.length
      );
      ...
      const averHR = Math.round(
        hrValues.reduce((sum, curr) => sum + curr, 0) / hrValues.length
      );
      // Quy đổi.
      ...
      const averTemp = Math.round(
        tempValues.reduce((sum, curr) => sum + curr, 0) / tempValues.length
      );
      ...
      const docterMessage = isHealthy
        ? "Tim bạn đang khỏe mạnh"
